...
function startGame() {
    myGamePiece = new component(35, 35, "images/penguin.png", 0, 100,0.4,0.25,"image");
    end_comp = new component(40, 40, "images/igloo.png", 1210, 400,0,0,"image");
    starter_comp = new component(22, 22, "red", 0, 100,0,0);

    for (i = 0; i < number_of_obstacles; i++) {
        if (i==0)
                    myObstacle.push(new component(20, 350, "red", 350 , 0,0,0));
                else if(i==1)
                    myObstacle.push(new component(20, 330, "red", 650 , 280,0,0));
                else if(i==2)
                    myObstacle.push(new component(20, 450, "red", 950 , 0,0,0));
            }
...
var myGameArea = {
            ...
            start: function() {
                ...
                window.addEventListener('mousemove', function(e) {
                    myGameArea.y = e.pageY * 1.05;         //multiplier can be used to control the speed of the y coordinate
                })
            }
            else{
                this.canvas.width = 1250;
...
//OBSTACLE DETECTON WITH THE LAST COMPONENT TO END THE GAME AND FORCE A RESTART
    if (end_comp.x - (end_comp.width / 2) <= gp.x && end_comp.x + (end_comp.width / 2) >= gp.x) {
        ...
        if (end_comp.y <= gp.y + gp.height && end_comp.y + end_comp.height >= gp.y) {
            ...
            //header = "SystemTime,\tSystemDate,\tGameTimer,\tX_Coordinate,\tY_Coordinate,\tScore\t,Hoizontal,\t,Vertical\n";
            //results.push(header);
            //save_result();
            //
...
function updateGameArea() {
    if (start_check == true) {
        ...
        coordinates.push(myGamePiece.y)
        coordinates.push(score,arr[0],arr[1],"\n")
